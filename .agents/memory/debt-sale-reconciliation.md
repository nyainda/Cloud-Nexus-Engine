...
**How to apply:** Keep server-side validation deployed so new nameless debt sales are rejected before writing. Treat any historical unmatched rows as a separate, user-confirmed data-repair task; when confirmed, create the missing debt with the sale's original timestamp and record an audit entry.