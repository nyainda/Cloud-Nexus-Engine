...
interface EditedItem {
  ...
  rawText: string;
  // AI-extracted clean product name used for matching (null if same as rawText).
  // This is saved as an alias when the user manually confirms a suggestion.
...
export default function OCR() {
  ...
  // ── Alias auto-learning ───────────────────────────────────────────────────
  // Tracks which (alias, productId) pairs have already been saved this session
  // to avoid duplicate POSTs when user clicks the same suggestion more than once.
  const savedAliasesRef = useRef<Set<string>>(new Set());

  // Fire-and-forget: saves an OCR alias for a product so future scans of the
  // same invoice format auto-match. Uses matchText (AI-cleaned name) when
  // available — avoids polluting aliases with price/qty suffixes from rawText.
  const saveAlias = useCallback((aliasText: string | null | undefined, productId: string | null | undefined) => {
    const alias = aliasText?.trim();
    if (!alias || !productId || alias.length < 3) return;
    const key = `${productId}::${alias.toLowerCase()}`;
    if (savedAliasesRef.current.has(key)) return;
    savedAliasesRef.current.add(key);
    // Fire-and-forget — errors are silently swallowed (alias learning is non-critical)
    customFetch(`/api/products/${productId}/aliases`, {
      method: "POST",
      body: JSON.stringify({ alias, autoGenerated: true, confidence: 0.9 }),
    }).catch(() => { /* non-fatal */ });
  }, []);
                                ...
                                {item.suggestions.slice(0, 5).map((s) => (
                                  <button
                                    ...
                                    onClick={() => {
                                      ...
                                      // Auto-learn: save the OCR text as an alias for this product
                                      // so future scans of the same invoice auto-confirm it.
                                      // Prefer matchText (AI-cleaned) over rawText (full invoice line).
                                      saveAlias(item.matchText ?? item.rawText, s.productId);
                                    }}
                                    className={cn(
