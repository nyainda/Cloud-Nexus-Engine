...
export function Light() {
  ...
  return (
    <div className="pos-light" style={{ background: "var(--bg)", color: "var(--fg)", height: "100svh", display: "flex", flexDirection: "column", fontFamily: "'DM Sans', sans-serif", overflow: "hidden" }}>

      {showCart ? (
        /* ── Cart Sheet ── */
        <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
          {/* Cart header */}
          <div style={{ padding: "16px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between", background: "var(--card)", flexShrink: 0, boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <ShoppingCart size={18} color="var(--primary)" />
              <span style={{ fontWeight: 700, fontSize: 15, color: "var(--fg)" }}>Cart</span>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "8px 0", background: "var(--bg)" }}>
            {cart.length === 0 ? (
              ...
              <div key={item.id} style={{ padding: "12px 16px", borderBottom: "1px solid var(--border)", background: "var(--card)", marginBottom: 2 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                  <div style={{ display: "flex", alignItems: "center", borderRadius: 10, border: "1px solid var(--border-strong)", overflow: "hidden", background: "var(--bg)" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                    <span style={{ fontSize: 10, color: "var(--fg-muted)", fontFamily: "'JetBrains Mono', monospace" }}>KSh</span>
          ...
          {/* Summary + checkout */}
          <div style={{ flexShrink: 0, borderTop: "1px solid var(--border)", background: "var(--card)", boxShadow: "0 -2px 8px rgba(0,0,0,0.06)" }}>
            <div style={{ padding: "14px 16px", display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                <span style={{ color: "var(--fg-muted)" }}>Subtotal ({cartCount} items)</span>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 13 }}>
                ...
                <input type="number" value={discount || ""} onChange={e => setDiscount(Number(e.target.value))} placeholder="0"
                  style={{ width: 88, height: 30, textAlign: "right", fontSize: 13, fontFamily: "'JetBrains Mono', monospace", background: "var(--bg)", border: "1px solid var(--border-strong)", borderRadius: 8, padding: "0 10px", color: "var(--fg)", outline: "none" }} />
              </div>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {[5, 10, 15, 20].map(pct => (
                  <button key={pct} onClick={() => setDiscount(Math.round(subtotal * pct / 100))}
                    style={{ fontSize: 10, fontWeight: 700, padding: "4px 10px", borderRadius: 99, background: discount === Math.round(subtotal * pct / 100) ? "var(--primary)" : "var(--bg)", color: discount === Math.round(subtotal * pct / 100) ? "var(--primary-fg)" : "var(--fg-muted)", border: `1px solid ${discount === Math.round(subtotal * pct / 100) ? "transparent" : "var(--border-strong)"}`, cursor: "pointer" }}>
                    {pct}%
                  </button>
                ))}
              </div>
            ...
            <div style={{ padding: "0 16px 12px" }}>
              <input type="text" placeholder="Customer name (required for debt)" style={{ width: "100%", height: 36, borderRadius: 10, border: "1px solid var(--border-strong)", background: "var(--bg)", color: "var(--fg)", fontSize: 13, padding: "0 12px", outline: "none", boxSizing: "border-box" }} />
            </div>

            {/* Checkout buttons */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, padding: "0 16px 20px" }}>
              <button style={{ height: 50, borderRadius: 14, background: "rgba(220,38,38,0.08)", border: "1.5px solid rgba(220,38,38,0.25)", color: "var(--destructive)", fontWeight: 700, fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                <CreditCard size={16} /> Debt
              </button>
              <button style={{ height: 50, borderRadius: 14, background: "var(--primary)", border: "none", color: "var(--primary-fg)", fontWeight: 800, fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, boxShadow: "0 4px 16px rgba(90,138,0,0.30)" }}>
        ...
        <>
          {/* Search header */}
          <div style={{ padding: "12px 12px 10px", background: "var(--card)", borderBottom: "1px solid var(--border)", flexShrink: 0, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
            ...
            <div style={{ display: "flex", gap: 6, overflowX: "auto" }}>
              {["All", "In Stock", "Low", "Out"].map((f, i) => (
                <button key={f} style={{ flexShrink: 0, padding: "5px 12px", borderRadius: 99, fontSize: 12, fontWeight: 600, cursor: "pointer", background: i === 0 ? "var(--primary)" : "var(--bg)", color: i === 0 ? "var(--primary-fg)" : "var(--fg-muted)", border: i === 0 ? "none" : "1px solid var(--border-strong)" }}>
                  {f}
                </button>
              ))}
            </div>
                ...
                return (
                  <button key={p.id} onClick={() => addToCart(p)} disabled={isOut}
                    style={{
                      ...
                      background: inCart ? "var(--card-active)" : isOut ? "rgba(0,0,0,0.02)" : "var(--card)",
                      border: `1.5px solid ${inCart ? "var(--primary-border)" : isOut ? "var(--border)" : "var(--border)"}`,
                      cursor: isOut ? "not-allowed" : "pointer", opacity: isOut ? 0.5 : 1,
                      transition: "all 0.12s", display: "flex", flexDirection: "column",
                      boxShadow: isOut ? "none" : "0 1px 4px rgba(0,0,0,0.06)"
                    }}>
                    {/* Top row: icon + status dot */}
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                      <div style={{ width: 30, height: 30, borderRadius: 9, background: p.weighed ? "rgba(90,138,0,0.10)" : "rgba(0,0,0,0.05)", display: "flex", alignItems: "center", justifyContent: "center", color: p.weighed ? "var(--primary)" : "var(--fg-muted)" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      ...
                      <div style={{ width: 24, height: 24, borderRadius: "50%", background: inCart ? "var(--primary)" : "rgba(0,0,0,0.07)", display: "flex", alignItems: "center", justifyContent: "center", color: inCart ? "var(--primary-fg)" : "var(--fg-muted)" }}>
          ...
          {cartCount > 0 && (
            <button onClick={() => setShowCart(true)}
              style={{ position: "fixed", bottom: 20, right: 16, left: 16, height: 54, borderRadius: 16, background: "var(--primary)", border: "none", color: "var(--primary-fg)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 16px", boxShadow: "0 6px 24px rgba(90,138,0,0.28)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
